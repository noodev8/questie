...
const { questManager } = require('../utils/database');
...
async function selectWeeklyQuests(excludeQuestIds = []) {
  const quests = [];
  ...
  for (const { level, count } of difficulties) {
    for (let i = 0; i < count; i++) {
      const quest = await questManager.getRandomQuestByDifficulty(level, excludeQuestIds);
      if (quest) {
        quests.push(quest);
...
router.get('/daily', authMiddleware.requireAuth, questLimiter, async (req, res) => {
  try {
    ...
    if (!dailyQuest) {
      // Assign a new random daily quest (medium difficulty)
      const randomQuest = await questManager.getRandomQuestByDifficulty('medium');
...
router.get('/weekly', authMiddleware.requireAuth, questLimiter, async (req, res) => {
  try {
    ...
    if (weeklyQuests.length === 0) {
      // Assign 5 new random weekly quests
      const selectedQuests = await selectWeeklyQuests();
...
router.post('/daily/reroll', authMiddleware.requireAuth, questLimiter, async (req, res) => {
  try {
    ...
    // Get a new random quest (excluding current one)
    const newQuest = await questManager.getRandomQuestByDifficulty('medium', [currentQuest.quest_id]);
...
router.post('/weekly/reroll', authMiddleware.requireAuth, questLimiter, async (req, res) => {
  try {
    ...
    // Select new weekly quests (excluding current ones)
    const newQuests = await selectWeeklyQuests(currentQuestIds);
